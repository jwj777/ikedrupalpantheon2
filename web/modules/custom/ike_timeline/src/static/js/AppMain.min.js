...
require(["underscore", "backbone", "app-view", "router", "app-config", "app-utils", "app-error"], function(_, Backbone, AppView, AppRouter, AppConfig, AppUtils, AppErrorReport) {
  return $(document).ready(function() {
    ...
    if (typeof Modernizr == 'undefined') $.loadScript('https://cdnjs.cloudflare.com/ajax/libs/modernizr/2.8.3/modernizr.min.js', function(){
    ...
    if ($('body').hasClass('mobile-handheld')) {
        if (!window.location.href.includes('mobile')) {
          if (window.location.href.endsWith('/timeline')) {
            window.location.href = window.location.href + '-mobile'
          }
          else if (window.location.href.endsWith('#')) {
            window.location.href = window.location.href.replace('#', '/mobile?wrap=true');
          }
          else {
            window.location.href = window.location.href + '/mobile'
          }
    ...
    return app.views.appView.on(app.views.appView.EVENT_DATA_LOADED, function() {
      ...
      if (fragment.trim() === "") {
        ...
        path = fragment.split("/");
        // remove timeline from path
        const index = path.indexOf('timeline');
        if (index > -1) {
          path.splice(index, 1);
        }
        if (path.length === 0) {
        ...
        const index_mobile = path.indexOf('timeline-mobile');
        if (index_mobile > -1) {
          path.splice(index_mobile, 1);
        }
        if (path.length === 0) {
          path = [""];
        }
