define(["jquery", "underscore", "backbone", "videojs", "fastdom", "lp_timeline/views/LPTimelineAbstractItemView"], function($, _, Backbone, vidJS, fastdom, LPTimelineAbstractItemView) {
  var LPTimelineModuleVideoPlayer;
  return LPTimelineModuleVideoPlayer = LPTimelineAbstractItemView.extend({
    _isPlaying: false,
    _isLoaded: false,
    _actions: [],
    events: {
    ...
    resize: function(h_, w_) {
      var _this = this;
      LPTimelineAbstractItemView.prototype.onResize(h_, w_);
      if (!this.modalText) {
        this.modalText = this.$el.find(".asset-modal-text");
      }
      if (!(this._video && this.$window && this.modalText)) {
        return;
      }
      return fastdom.measure(function() {
        var h, maxH, mt, r, w;
        if (w_ == null) {
          w_ = _this.$window.width();
        }
        if (h_ == null) {
          h_ = _this.$window.height();
        }
        maxH = _this.$window.height() - _this.modalText.height() - 170;
        r = 9 / 16;
        w = w_ * (7 / 8);
        h = w * r;
        if (h > maxH) {
        ...
        return fastdom.mutate(function() {
          var p;
          if ($(_this._video.el).hasClass("fullscreen")) {
            p = {
              "width": "",
              "height": "",
              "margin-top": ""
    ...
    */
    ...
    setVideo: function(videoURL, captionsURL, metadata) {
      ...
      return this._video.ready(function() {
        ...
        _this.onResize(_this.$window.height(), _this.$window.width());
        _results = [];
    ...
    initialize: function(args) {
      ...
      return app.utils.delay(100, function() {
        return _this.onResize(_this.$window.height(), _this.$window.width());
      });
    },
    destroy: function() {
      this.params = null;
    ...
    _onPlayerReady: function(videoEl) {
      ...
      this.onResize(this.$window.height(), this.$window.width());
      _results = [];
